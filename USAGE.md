...
正式工具提供 `prepare --task-packet … --brief`、事件去重、按游标读取和按需恢复。`--brief` 减少返回给调度方的重复任务正文，子 agent 仍收到完整契约。注意 `runs.py recover --limit` 目前只分页待办，不限制全部 jobs/rounds/watches；精简恢复与 delta 查询已列入待办。
