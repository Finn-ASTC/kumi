...
To inspect registered work sessions without reading transcripts or changing host history, run `python3 scripts/sessions.py --run "$RUN_FILE" --limit 100 --offset 0`. This is a read-only directory and resume/archive preview. Explicit identities and parent relationships are kept in the run's kumi-owned `session-registry` sidecar; host capability evidence is kept separately in `capability-registry`. Register host + store/profile + exact session ID and the role/relationship before claiming a native binding. Host, profile/store identity, roles, parent relationships, or capabilities that were not recorded remain `unknown`; only verified capability evidence can clear a dry-run blocker, and a preview is never permission to resume or archive. External user sessions are unowned records.
...
In an identified herdr session, default to a **new tab in the controller's parent workspace**, using `--no-focus` to keep the controller's current page. A new workspace remains an explicit layout choice. Split the parent tab only on an explicit request for a split/side-by-side layout. Record the actual workspace/tab/pane IDs and give the exact viewing command; a selectable page is not an already focused page. For isolated sessions or an unknown parent, use a dedicated workspace and provide the attach command. See the [display policy](references/transports.md#display-and-viewing). Viewing an existing task must not launch another agent or resubmit its prompt.

Name each new workspace/tab with the task and host type, in the user's language: for example `实现上传重试 · Codex` or `验证上传兼容性 · OpenCode (OMO)`. Add a short discriminator for duplicate purposes. Keep the unique internal agent name separate from this readable label. Related follow-ups retain their page and label. See [page naming](references/transports.md#name-pages-by-their-task-and-host) for creation, initial-tab naming and owned-page renaming.
