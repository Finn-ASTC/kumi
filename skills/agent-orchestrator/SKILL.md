...
To inspect registered work sessions without changing host history, run `python3 scripts/sessions.py --run "$RUN_FILE" --limit 100 --offset 0`. See [session records and previews](references/sessions.md) for explicit identity/relationship registration and evidence commands. Missing identities remain unknown; multiple active-round records are ambiguous. Capability claims are checked against pinned evidence bytes, not certified against the live host. Resume/archive eligibility remains false until runtime checks and adapters exist. External user/coordinator records grant no control.
