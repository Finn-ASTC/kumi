...
def _session_row(run: dict[str, Any], state: dict[str, Any]) -> dict[str, Any]:
    ...
    native_state = "unbound" if session_id is None else "host_unknown"
    archive_reasons = []
    if closed is None:
    ...
    return {
        "group": "worker", "role": "unknown", "parent_job_id": None,
        "relationship_status": "not_recorded",
        ...
        "closed": closed,
        "native_identity": {
            "host": None, "profile": profile, "session_id": session_id,
            "status": native_state, "launcher": executable,
        },
        "resources": resources,
        "request_path": str(request_path),
        "recovery_preview": {
            "available": False, "candidate": session_id is not None,
...
def directory(run_path: str, *, job_id: str | None = None, limit: int = 100,
    ...
    for path in selected:
        ...
        try:
            ...
            rows.append(_session_row(run, state))
    ...
    return {
        ...
        "note": "Main coordinator sessions and parent-child links require explicit registration; none are inferred.",
    }
