...
消息技术交付见[M1 规格](M1-IMPLEMENTATION-PLAN.md)：包含持久未读、防遗忘提醒、
收口竞态、可替换通知适配及 hcom 小型选择试验。全部排期、状态和下一单元只在
[综合队列](IMPLEMENTATION-PLAN.md#下一阶段执行队列)维护；hcom 仍是未安装候选。

后续以[DataHive 成本与质量复核](DATAHIVE-COST-QUALITY.md)约束优化与评测：独立验证有
实际检出价值，调度长会话也有可观测成本。计量贯穿原生实验，验收规范提前使用；
文件消息可以先开发，不要求先实现全部接班。CQ-03 排在 M3/M4 前，不预设新会话更省。
...
下表是 M0–M4 的能力依赖，不单独规定跨专题排期；实际执行以综合队列为准。
